function registerSettings() {
	...
	game.settings.register("foundry-pf2e-vault-weapon-masters", "popupVis", {
		name: "One-Time Popup",
		scope: "client",
...
async function openIntroduction (){
	const journals = await game.packs.get("foundry-pf2e-vault-weapon-masters.vault-weapon-masters-journals").getDocuments();
	const introduction = journals.filter(e => e.id == "CGM7KN8WMUMcHSl1")[0]
	introduction.sheet.render(true)
	game.settings.set("foundry-pf2e-vault-weapon-masters", "popupVis", false)
}

Hooks.once('ready', async function () {
    if (game.user.isGM) {
        if (game.settings.get("foundry-pf2e-vault-weapon-masters", "popupVis") == true) {
            let d = new Dialog({
                ...
                <p>
								...
								<p>
								This Foundry VTT module has been prepared by Braden "Delioth" Menke of Eldritch Osiris Games, please report any bugs <a href="https://github.com/Eldritch-Osiris-Games/foundry-pf2e-vault-weapon-masters/issues">here!</a>
								</p>
                `,
                buttons: {
									one: {
                    ...
                    two: {
                        ...
                        label: "Close",
                        callback: () => game.settings.set("foundry-pf2e-vault-weapon-masters", "popupVis", false)
                    },
                },
            },{ width: 450});
            d.render(true);
